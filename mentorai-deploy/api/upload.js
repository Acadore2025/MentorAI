...
async function extractTextFromPDF(pdfBuffer, filename) {
  ...
  const purposePart = Buffer.from(
    `\r\n--${boundary}\r\nContent-Disposition: form-data; name="purpose"\r\n\r\nassistants\r\n--${boundary}--\r\n`
  );
  const body = Buffer.concat([header, pdfBuffer, purposePart]);
  ...
  if (!uploadData.id) {
    throw new Error(`OpenAI file upload failed: ${uploadData.error?.message || JSON.stringify(uploadData)}`);
  }

  const fileId = uploadData.id;
  console.log(`[UPLOAD] File uploaded to OpenAI: ${fileId}`);

  // Step 2: Extract text using GPT-4o with the uploaded file
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          {
            type: 'text',
            text: 'Extract ALL text from this NCERT PDF. Return clean plain text only. Preserve headings and paragraph structure. No markdown or special formatting.'
          },
          {
            type: 'file',
            file: { file_id: fileId }
  ...
  // Cleanup: delete file from OpenAI after extraction
